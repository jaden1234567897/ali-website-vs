...
// ali-v6: classic 3-blade asterisk — the shape from image 2. All three
// coins edge-on (rx = +90°), arranged at z-rotations 0°/+60°/-60° so the
// blades fan into a clean 6-pointed star. Tiny y offsets (±0.05) and
// outer-coin z offset (-0.40 vs middle's -0.34) give a hint of depth so
// the three blades don't perfectly z-fight. Mirrored verbatim into
// AliBridge.starPoses so the swap is identical.
...
// ali-v5: coastal teal/navy trio, mapped darkest → lightest by phase so
// the visual reads as depth → analysis → emerging clarity and pairs with
// the new site palette.
//   STRATEGY  → deep navy   #1B4965   (foundational depth)
//   AI        → mid-teal    #62B6CB   (analytical bridge)
//   EXECUTION → pale teal   #BEE9E8   (emerging clarity)
const coinColors = ['#1B4965', '#62B6CB', '#BEE9E8']

// Label fill per coin for legibility against each tint:
//   navy      + white     ≈ 9.2:1   ✓
//   mid-teal  + white     ≈ 3.0:1   ✓ (large display text)
//   pale teal + navy ink  ≈ 8.6:1   ✓
const coinLabelColors = ['#ffffff', '#ffffff', '#1B4965']

declare global {
  interface Window {
    __aliNovaProgress?: number
    // ali-v6: AliBridge writes its pinned-scroll progress here so CoinField
    // can tie its fade-out directly to AliBridge's internal timeline
    // instead of estimating via section geometry.
    ...
    async function boot() {
      ...
      const updateScrollProgress = () => {
        ...
        if (canvasRef.current) {
          ...
          // ali-v6: keep CoinField alive while AliBridge is in its text +
          // asterisk-hold phase, then fade out FULLY before the triangle
          // unroll begins so the user sees AliBridge's smooth detach
          // animation (asterisk → triangle) unobstructed by any CoinField
          // overlay. Tied directly to AliBridge's pinned-scroll progress:
          //   bridge progress 0.00 → 0.34 → CoinField fully visible
          //   bridge progress 0.34 → 0.41 → CoinField fades out
          //   bridge progress 0.41+      → CoinField gone (phase C plays clean)
          //
          // Before AliBridge's pin starts the value is 0, which still keeps
          // CoinField visible — so the hero-to-bridge entry stays smooth.
          // We also need the bridge section to be in view at all for this
          // factor to apply (otherwise later sections would keep the coins
          // alive across the whole page).
          let bridgeKeepAlive = 0
          const bridge = document.querySelector<HTMLElement>('.ali-bridge-section')
          if (bridge) {
            const bridgeRect = bridge.getBoundingClientRect()
            const bridgeInArea =
              bridgeRect.top < window.innerHeight && bridgeRect.bottom > 0
            if (bridgeInArea) {
              const bridgeProgress = window.__aliBridgeProgress ?? 0
              // ali-v6: TRUE step function (no smoothstep window at all).
              // bridgeKeepAlive is binary 1 or 0 — at the exact instant
              // bridgeProgress crosses 0.40, CoinField goes from opacity
              // 0.98 to 0 in the SAME frame AliBridge's canvas snaps to
              // opacity 1 (via tl.set, no duration). Zero overlap window
              // = zero doubled-blade artifact.
              bridgeKeepAlive = bridgeProgress < 0.40 ? 1 : 0
            }
          }

          targetCanvasOpacity.current = Math.max(
            0.92 * heroVisibility,
            transitionVisible ? 0.98 : 0,
            0.98 * bridgeKeepAlive,
          )
        }
      }
      ...
      new GLTFLoader().load('/silver_coin.glb', gltf => {
        ...
        initialPoses.forEach((_, index) => {
          ...
          // ali-v4: tint each coin with its own solid base color. The GLB's
          // original metalness/roughness are left untouched so the coin keeps
          // the same polished-metal response as the hero — only the color
          // changes. proto.clone(true) clones the scene graph but materials
          // are still shared by reference, so we clone every Mesh's material
          // before tinting — otherwise coin #0's color would also recolor
          // coins #1 and #2.
          const tint = new THREE.Color(coinColors[index])
          const applyTint = (m: import('three').Material) => {
          ...
          model.traverse(child => {
            ...
            if (Array.isArray(material)) {
              mesh.material = material.map(applyTint)
            } else if (material) {
              mesh.material = applyTint(material)
            }
          })
          ...
          const label = new THREE.Mesh(
            ...
            new THREE.MeshBasicMaterial({
              map: createLabelTexture(coinLabels[index], coinLabelColors[index]),
              transparent: true,
              depthWrite: false,
      ...
      const render = () => {
        ...
        // ali-v6: coins gather on first scroll (close=0.02→0.40 of hero).
        // Roll runs from 0 → 2 across scrolls 1-3 so the asterisk spins
        // exactly 2 full revolutions (720°) by the time the text phase
        // ends. Distribution:
        //   hero    scroll 0.00 → 1.00 contributes 0 → 1 to roll
        //   bridge  prog   0.00 → 0.40 contributes 0 → 1 to roll
        // Multiplier of Math.PI × 2 below converts roll-unit-to-radians
        // so the visible spin lands at exactly 720° at scroll 3 end.
        // ali-v6: tightened gather window (was 0.02 → 0.40). Now the
        // asterisk forms FULLY by scroll 0.15 (about 2 wheel notches into
        // the hero). Before this fix, the gather took until scroll 0.40 to
        // complete, so the user would see coins as ellipses (mid-rotation
        // between face-on initial pose and edge-on butterfly) for the
        // first ~7 wheel notches — looked broken until far into the hero.
        const close = smoothstep(0.02, 0.15, scroll)
        // ali-v6: roll caps at exactly 2 (= 4π rotation = 720° = 0 mod 360°)
        // by bridge progress 0.35, then HOLDS at 2 from 0.35 → 0.40. The
        // 0.05 "settled" buffer guarantees the asterisk is static at angle
        // 0 by the time CoinField hands off to AliBridge at progress 0.40,
        // eliminating the sub-degree rotation mismatch that was causing
        // the small visible "snap" during the reverse/detach handoff.
        const bridgeText = clamp(window.__aliBridgeProgress ?? 0, 0, 0.35)
        const bridgeRollPhase = bridgeText / 0.35
        const roll = reduced ? 0 : scroll + bridgeRollPhase
        ...
        if (canvasRef.current) {
          // ali-v6: bypass the easedCanvasOpacity smoothing and write the
          // target directly. The smoothstep on heroVisibility already
          // provides smooth fading for the hero, and we need the bridge
          // swap (smoothstep 0.400 → 0.401) to be a hard cut. Easing was
          // turning the hard cut into a visible 150ms fade — causing the
          // doubled-asterisk artifact during the swap.
          canvasRef.current.style.opacity = (targetCanvasOpacity.current * canvasFade).toFixed(3)
        }
        ...
        if (novaActive) {
          ...
          rig.position.set(0, 0, -0.02)
          rig.scale.setScalar(1)
          // ali-v6: 2 full turns (720°) total across scrolls 1-3. `roll`
          // runs 0 → 2 (hero contributes 1, bridge text phase contributes
          // another 1), and the Math.PI × 2 multiplier here makes each
          // unit of roll = 360°. So total z-rotation = 2 × Math.PI × 2 = 4π = 720°.
          // The X/Y sin-wobble is dampened (roll/2) so it still returns
          // cleanly to 0 at the asterisk-hold end.
          rig.rotation.set(
            (reduced ? 0 : Math.sin((roll / 2) * Math.PI) * 0.1) - pointer.y * 0.055 * hoverStrength,
            (reduced ? 0 : Math.sin((roll / 2) * Math.PI) * 0.16) + pointer.x * 0.075 * hoverStrength,
            roll * Math.PI * 2,
          )
        }
