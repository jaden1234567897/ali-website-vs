...
// ali-v6: classic 3-blade asterisk (image 2 target). Mirrored verbatim
// from CoinField.butterflyPoses so the hero → bridge swap lands on the
// IDENTICAL shape.
const starPoses = [
  { x: 0, y:  0.05, z: -0.40, s: 1.5, rx: Math.PI * 0.5, ry:  0.04, rz: Math.PI / 3 },
  { x: 0, y:  0.00, z: -0.34, s: 1.5, rx: Math.PI * 0.5, ry:  0,    rz: 0 },
  { x: 0, y: -0.05, z: -0.40, s: 1.5, rx: Math.PI * 0.5, ry: -0.04, rz: -Math.PI / 3 },
]
...
// ali-v5: per-coin base color, kept in lockstep with CoinField.tsx so the
// bridge section's coins render in the same navy/teal trio as the hero
// instead of falling back to the GLB's default silver.
//   STRATEGY  → deep navy   #1B4965
//   AI        → mid-teal    #62B6CB
//   EXECUTION → pale teal   #BEE9E8
const COIN_COLORS = ['#1B4965', '#62B6CB', '#BEE9E8']

// ali-v5: per-coin engraved-label fill color. The bridge section reuses
// the same per-coin contrast rules as the hero — white labels on the two
// darker metals (navy + mid-teal), navy ink on the pale-teal coin. The
// previous single hardcoded '#202328' rendered the STRATEGY label nearly
// invisible against the deep-navy face.
...
const styles = {
  ...
  paragraph: {
    ...
    maxWidth: 980,
    textAlign: 'center',
    // ali-v5: weight + tracking aligned with the hero h1 (Literata 600 @
    // letter-spacing 0). Bridge previously used 400 / -0.035em which is a
    // visibly thinner Literata cut — the hero's heavier weight reads as a
    // different font even though the family is identical.
  ...
  // ali-v4: slide images replace the Strategy/AI left-column text. The
  // wrapper (slideFrame) carries the high-end card treatment — rounded
  // corners, layered ambient + contact shadow, a subtle perspective tilt
  // that reads as a "floating product preview". The inner image keeps
  // `mix-blend-mode: multiply` so the slide's solid-white background
  // dissolves into the cream site base, leaving only the table and text
  // floating inside the rounded card silhouette.
  //
  // Box-shadow has to live on the wrapper, not the image — mix-blend-mode
  // on an element suppresses its own box-shadow rendering, so the shadow
  // would vanish if we put both on the <img>.
  slideFrame: {
    position: 'relative',
    width: '100%',
    borderRadius: 14,
    ...
    boxShadow:
      // Deep ambient — soft, far, low opacity (the "floating" cue)
      '0 40px 80px -28px rgba(28, 38, 60, 0.22), ' +
      // Mid bounce — tighter, gives the card weight
      '0 16px 32px -12px rgba(28, 38, 60, 0.14), ' +
      // Contact — very sharp/short, sells "card sitting above page"
      '0 2px 6px -1px rgba(28, 38, 60, 0.08), ' +
      // Hairline edge — barely-there ring substitutes for a border without
      // creating the "boxed image" look the user explicitly rejected
      '0 0 0 1px rgba(28, 38, 60, 0.04)',
    willChange: 'transform, opacity, filter',
  } as const,
  slideImage: {
    display: 'block',
...
// ali-v4: Strategy and AI left columns are now Ali's oria slides instead of
// bullet text. Reveal/fade is driven by the same GSAP keys (leftHeaderRef,
// aiHeaderRef) that used to target each column's h3 — the image now wears
// those refs directly, and the bullet arrays simply stay empty.
...
export default function AliBridge() {
  ...
  // ali-v4: left + AI columns are images, not bullet lists, so only the
  // right column still uses a bullet-refs array.
  const rightBulletsRef = useRef<(HTMLLIElement | null)[]>([])
  // ali-v4: left + AI columns now wrap images, right column still wraps an
  // h3. HTMLElement keeps GSAP happy across all three (it only needs a DOM
  // node) without re-typing every consumer.
  const leftHeaderRef = useRef<HTMLElement>(null)
  const rightHeaderRef = useRef<HTMLHeadingElement>(null)
  const aiHeaderRef = useRef<HTMLElement>(null)
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // ali-v4: leftBulletsRef and aiBulletsRef arrays are empty (slides
      // replaced the bullet text). rightBulletsRef still drives the "My
      // role" reveal on the Execution phase.
      gsap.set(rightBulletsRef.current, { opacity: 0, y: 18, filter: 'blur(8px)' })

      // ali-v6: canvas starts INVISIBLE so the bridge's text phase shows
      // CoinField's coins (which are already in butterfly pose from the
      // hero) instead of double-rendering with this section's own scene.
      // Crossfade to AliBridge's canvas happens during the late asterisk
      // hold (just before phase C unrolls the coins into the triangle).
      ...
      // ali-v6: hard step at bridge progress 0.40 — tl.set is instant
      // (no duration). Paired with CoinField using a sharp step in the
      // opposite direction, only ONE canvas is ever visible. No crossfade
      // window means no doubled-blades / dual-asterisk artifact during
      // the swap. Pose data + rotation angle match at 0.40, so the swap
      // is visually identical.
      tl.set(canvasRef.current, { opacity: 1 }, 0.40)
      ...
      // ── Phase E: Strategy zoom, slide 4 reveals on LEFT ──
      // ali-v4: the column's text was replaced by the oria-slide-4 image,
      // which now carries leftHeaderRef directly. The bullet stagger that
      // used to follow the header is gone — the whole slide reveals as one
      // element. Duration bumped slightly so the image's blur-up reads as
      // a single graceful motion rather than the original snappy two-stage.
      tl.to(
        leftHeaderRef.current,
        { opacity: 1, y: 0, filter: 'blur(0px)', duration: 0.06 },
        0.69,
      )
      // ── End Phase E: Strategy + slide fade out together ──
      tl.to(
        leftHeaderRef.current,
      ...
      // ── Phase F (ali-v4): AI solo — slide 5 reveals on LEFT, same
      //    blur-up animation. Single-element reveal/hold/fade, no bullet
      //    stagger now that the bullets are replaced by one image.
      tl.to(
        aiHeaderRef.current,
        { opacity: 1, y: 0, filter: 'blur(0px)', duration: 0.06 },
        0.84,
      )
      // AI slide fades out at the same beat the Execution coin starts rising.
      tl.to(
        aiHeaderRef.current,
    ...
    async function boot() {
      ...
      new GLTFLoader().load('/silver_coin.glb', gltf => {
        ...
        COIN_LABELS.forEach((labelText, index) => {
          ...
          //
          // ali-v4: while we're cloning, also tint each material with the
          // per-coin base color from COIN_COLORS so this section's coins
          // render in the same gold/silver/copper as the hero. The GLB's
          // original metalness/roughness are left untouched — only color
          // changes, matching CoinField.tsx's behaviour exactly.
          const tint = new THREE.Color(COIN_COLORS[index])
          const tintMaterial = (m: import('three').Material) => {
            const cloned = m.clone() as import('three').MeshStandardMaterial
          ...
          const labelMesh = new THREE.Mesh(
            ...
            new THREE.MeshBasicMaterial({
              map: createLabelTexture(labelText, COIN_LABEL_COLORS[index]),
              transparent: true,
              depthWrite: false,
  ...
  return (
    ...
    >
      <div style={styles.stage}>
        ...
        {/* ali-v4: Strategy phase — oria slide 4 inside a premium card frame.
            The frame carries the GSAP ref so the entire card (shadow,
            perspective, image) blur-ups and fades as one element. The
            inner image uses mix-blend-mode so its white background still
            dissolves into the cream page bg. */}
        {/* ali-v6: Strategy slide column gets a small section title above
            the slide ("AI in strategy execution") and the whole column is
            lifted from vertical-centre (50%) to 38% so the title sits in
            comfortable air above the slide instead of pushing the slide
            into the hero. The title shares the GSAP reveal/fade ref with
            the slide so they appear and disappear together as one beat. */}
        <div
          ref={leftColRef}
          className="ali-bridge-col ali-bridge-col--left ali-bridge-col--slide ali-bridge-col--slide-titled"
          style={styles.leftColumn}
        >
          {/* ali-v6: title + slide wrapped in a single GSAP-controlled
              block (leftHeaderRef now points to this wrapper, not the
              inner slide-frame). Both children fade/blur/y together —
              the title no longer leaks through the Bridge text phase
              or the AI/Execution phases that follow. The inner
              .ali-slide-frame keeps its perspective tilt unchanged. */}
          <div
            ref={leftHeaderRef as unknown as React.RefObject<HTMLDivElement>}
            className="ali-bridge-strategy-block"
          >
            <h3 className="ali-bridge-slide-title">AI in strategy execution</h3>
            <div className="ali-slide-frame" style={styles.slideFrame}>
              <img
                src="/oria-slide-4.png"
                alt="Customer Journey: Improvement Initiatives Across Each Step"
        ...
        {/* ali-v4: AI phase — oria slide 5 in the same premium frame. The
            frame tilts the OPPOSITE direction (positive rotateY) so the
            two slides feel like a matched pair across the section rather
            than the same card shown twice. */}
        <div
          ref={aiColRef}
          className="ali-bridge-col ali-bridge-col--left ali-bridge-col--slide"
          style={styles.leftColumn}
        >
          <div
            ref={aiHeaderRef as unknown as React.RefObject<HTMLDivElement>}
            className="ali-slide-frame ali-slide-frame--mirror"
            style={styles.slideFrame}
          >
            <img
              src="/oria-slide-5.png"
              alt="Workstream: Data Platform charter example"
              style={styles.slideImage}
              draggable={false}
            />
          </div>
      ...
      {/* Mobile layout overrides — coin sits at the top of the section,
          ...
          ali-v4: the .ali-bridge-col--slide variant carries an image rather
          than text, so it opts out of the text-only max-height clip and
          centers the image instead of left-aligning it. */}
      <style jsx>{`
        /* ali-v4: AI phase mirrors the Strategy tilt so the two cards feel
           like a matched pair across the section, not duplicate stagings.
           Inline transform wins by default, so we override here with the
           same perspective stack but flipped rotateY. */
        :global(.ali-slide-frame--mirror) {
          transform: perspective(1400px) rotateY(2.5deg) rotateX(0.5deg) !important;
        }
        /* ali-v6: Strategy slide column gets its own variant: lift the
           column from top:50% to top:38% so the title above the slide
           has room to breathe, and pop the title style. */
        :global(.ali-bridge-col--slide-titled) {
          top: 38% !important;
        }
        :global(.ali-bridge-slide-title) {
          margin: 0 0 clamp(12px, 1.2vw, 22px) 0;
        ...
        @media (max-width: 768px) {
          ...
          /* Flatten the perspective tilt on mobile — at narrow widths the
             rotation steals horizontal space and the card edge clips. */
          :global(.ali-bridge-col--slide .ali-slide-frame) {
            transform: none !important;
            border-radius: 10px;
          }
          :global(.ali-bridge-col--slide img) {
            max-height: 44vh;
            width: 100%;
            height: auto;
            object-fit: contain;
